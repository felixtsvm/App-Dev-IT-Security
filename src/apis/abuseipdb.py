...
try:
    # Versucht die Cloud-Secrets zu lesen, falls die App auf Streamlit Cloud geöffnet wird
    api_key = st.secrets['AbuseIPDB_KEY']
except Exception:
    # Falls die Cloud-Secrets nicht verfügbar sind (z. B. bei lokalem Testen), wird der API-Key aus den Umgebungsvariablen gelesen
    api_key = os.getenv('AbuseIPDB_KEY')


def get_abuseipdb_info(ip_address):
    """
    Sendet eine IP-Adresse an die AbuseIPDB-API (abuseipdb.com), um Informationen 
    ...
    parameters = {
        ...
        # Rückschau-Zeitraum für die Überprüfung (max. 90 Tage) - Lang genug, um Angriffe zu erkennen, 
        # aber kurz genug, um Fehlalarme durch IP-Wechsel der Provider zu verhindern
        'maxAgeInDays': 90
    }
    ...
    try:
        ...
        if response.status_code == 200:
            # .json()['data'] öffnet die äußere 'data'-Hülle der API-Antwort, damit app.py sofort auf den Score, das Land etc. zugreifen kann
    ...
    # Wenn im try-Block ein Fehler auftritt, greift dieser except-Block und fängt die Ausnahme ab
        ...
        return {'error': f'Anfragefehler AbuseIPDB: {str(e)}'}